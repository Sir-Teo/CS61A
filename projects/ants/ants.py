...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            self.digesting -= 1
        elif self.place.bees is not None and len(self.place.bees) != 0 and self.digesting == 0:
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if hasattr(self.contained_ant,'action'):
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_true_queen:          #haha, it dies
            ...
            while this_place.exit is not None:
                if this_place.ant is not None:
                    this_ant = this_place.ant
                    if self.is_true_queen:
                        if (not isinstance(this_ant,QueenAnt)) and not this_place.ant in QueenAnt.doubled:
                            this_ant.damage *= 2
                            QueenAnt.doubled.append(this_ant)
                        if isinstance(this_ant, BodyguardAnt):
                            if (this_ant.contained_ant) and (not this_ant.contained_ant in QueenAnt.doubled):
                                this_ant.contained_ant.damage *= 2
                                QueenAnt.doubled.append(this_ant.contained_ant)
                if this_place.exit is not None:
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor and self.is_true_queen:
            bees_win()
        # END Problem 13
