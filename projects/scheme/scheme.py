...
class Procedure:
    ...
    def eval(self, operands, env):
        eval_operands = lambda expr: scheme_eval(expr, env)
        return scheme_apply(self, operands.map(eval_operands), env) 
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval(self, operands, env):
        ...
        return scheme_eval(complete_evaluate(applied), env)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if scheme_falsep(first_expr):  
            return False
        elif scheme_truep(first_expr): 
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            if (scheme_symbolp(first) and first in SPECIAL_FORMS):
                ...
                procedure = scheme_eval(first,env)
                check_procedure(procedure)
                result = procedure.eval(rest, env)
                #args = rest.map(lambda fun: scheme_eval(fun,env))
                #result = complete_apply(procedure,args,env)
        return result
        # END PROBLEM 20
