...
; expect (a b c a d e)
(compress '(a a b c c a a d s e w d e e e e))
; expect (a b c a d s e w d e)
(compress '(a a e t t t t t t t t d d e e e e))
; expect (a e t d e)
(compress '(c c c s e e w d f g e))
; expect (c s e w d f g e)
  ...
  (if (null? liste)
      ...
      (let ((packed-cdr (pack (cdr liste))))
        (cond ((null? (car packed-cdr))               ; packed-cdr provient du cas trivial
               (list (list (car liste))))
              ((equal? (car liste) (car (car packed-cdr))) ; même élément: ajout à la sous-liste
               (cons (cons (car liste)
                           (car packed-cdr))
                     (cdr packed-cdr)))
...
(cons (cons 6 (cons 1 nil)) '(a))
; expect ((6 1) a)
`(list ,(cons 1 2) (list 3 4))
; expect (list (quote (cons 1 2)) (list 3 4))
(car (cdr-stream s))
; expect okay
